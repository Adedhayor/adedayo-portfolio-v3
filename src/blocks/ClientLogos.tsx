// ============================================================
// ClientLogos block — a "worked with" carousel using the original
// monochrome logo marks (currentColor, theme-aware). Scrolls
// horizontally on a continuous loop with soft fades at both edges
// (About feedback 2026-07-12 #3); pauses on hover.
// ============================================================
  ...
  return (
    <div aria-hidden={ariaHidden || undefined} className="flex shrink-0 items-center gap-opt-4xl pr-opt-4xl">
      {CLIENT_LOGOS.map(({ name, Mark }) => (
        <span
          key={name}
          className="inline-flex items-center gap-2 whitespace-nowrap text-opt-text-placeholder transition-colors duration-[var(--opt-motion-base)] hover:text-opt-text-heading"
        >
